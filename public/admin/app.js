...
function renderReviews() {
  ...
  content.reviews.items.forEach(function(item, i) {
    ...
    html += '<div class="field"><label>Рейтинг (1-5)</label><input type="number" min="1" max="5" value="' + esc(item.rating) + '" data-change="set-review" data-i="' + i + '" data-key="rating"></div>';
    html += '</div>';
  });
...
function renderAchievements() {
  ...
  content.achievements.items.forEach(function(item, i) {
    ...
    html += '<div class="field" style="flex:1;min-width:60px"><label>Top (%)</label><input type="number" value="' + esc(item.top !== undefined ? item.top : '') + '" data-change="set-achievement" data-i="' + i + '" data-key="top"></div>';
    html += '<div class="field" style="flex:1;min-width:60px"><label>Left (%)</label><input type="number" value="' + esc(item.left !== undefined ? item.left : '') + '" data-change="set-achievement" data-i="' + i + '" data-key="left"></div>';
    html += '<div class="field" style="flex:1;min-width:60px"><label>Right (%)</label><input type="number" value="' + esc(item.right !== undefined ? item.right : '') + '" data-change="set-achievement" data-i="' + i + '" data-key="right"></div>';
    html += '<div class="field" style="flex:1;min-width:60px"><label>Bottom (%)</label><input type="number" value="' + esc(item.bottom !== undefined ? item.bottom : '') + '" data-change="set-achievement" data-i="' + i + '" data-key="bottom"></div>';
    html += '</div>';
    html += '<div style="display:flex;gap:0.5rem">';
    html += '<div class="field" style="flex:1"><label>Rotate (deg)</label><input type="number" value="' + esc(item.rotate !== undefined ? item.rotate : '') + '" data-change="set-achievement" data-i="' + i + '" data-key="rotate"></div>';
    html += '<div class="field" style="flex:1"><label>Duration (s)</label><input type="number" step="0.1" min="1" value="' + esc(item.duration !== undefined ? item.duration : '') + '" data-change="set-achievement" data-i="' + i + '" data-key="duration"></div>';
    html += '</div>';
    html += '</div>';
  });
